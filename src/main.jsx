...
function App() {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      setAnalysisResult(data); setAnalysisComplete(true)
    } catch (error) { setAnalysisError(error.message) } finally { setAnalysis(false) }
  }

  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark"><Sparkles size={17} /></div><span>ProductSignal</span></div><nav>{nav.map(([label, Icon]) => <button key={label} className={page === label ? 'nav-item active' : 'nav-item'} onClick={() => setPage(label)}><Icon size={17} /> {label}</button>)}</nav><div className="sidebar-bottom"><div className="workspace-label">WORKSPACE</div><div className="workspace"><div className="avatar">PM</div><div><strong>Product team</strong><span>Discovery workspace</span></div></div></div></aside>
...
function Overview({ feedback, analysis, analysisComplete, analysisResult, analysisError, runAnalysis, setPage }) { const themes = analysisResult?.themes?.length ? analysisResult.themes : null; return <><section className="summary-grid"><Stat icon={MessageSquareText} label="Feedback analyzed" value={feedback.length} note="customer evidence" /><Stat icon={BrainCircuit} label="Problems identified" value={themes?.length || 3} note="evidence-backed themes" /><Stat icon={Lightbulb} label="Opportunities" value={analysisResult?.opportunities?.length || 3} note="ready for judgment" /></section><section className="content-grid"><div className="panel themes-panel"><div className="panel-heading"><div><h2>Top customer themes</h2><p>What customers are telling you most often.</p></div><button className="text-button" onClick={() => setPage('Feedback')}>View feedback <ChevronRight size={15} /></button></div><div className="theme-list">{(themes || [['Too much manual work','Customers describe repetitive administrative tasks and duplicate entry.','High'],['Reporting is hard to act on','Customers can access data but struggle to turn it into useful decisions.','High'],['Need access on the go','Customers want to complete common tasks away from their desk.','Medium']]).slice(0,3).map((t,i)=><div className="theme" key={t.name || t[0]}><div className="theme-number">0{i+1}</div><div className="theme-body"><div className="theme-title">{t.name || t[0]} <span className={(t.confidence || t[2]) === 'High' ? 'pill high' : 'pill'}>{t.confidence || t[2]} confidence</span></div><p>{t.description || t[1]}</p><div className="evidence"><span>{t.evidenceIds?.length || 0} linked items</span><span>•</span><span>review evidence</span></div></div></div>)}</div></div><div className="panel ai-panel"><div className="ai-header"><div className="ai-icon"><BrainCircuit size={18}/></div><div><h2>AI analysis</h2><p>Helping you find signal, not make the decision.</p></div></div><div className="ai-callout"><Sparkles size={16}/><div><strong>{analysis?'Analyzing feedback...':analysisError?'AI analysis unavailable':analysisComplete?'Analysis complete':'Ready to analyze'}</strong><p>{analysis?'Looking for recurring problems, evidence, and gaps in understanding.':analysisError?analysisError:analysisComplete?'Analysis is grounded in the feedback IDs supplied. Review the evidence before making a product decision.':'Run AI analysis against the current feedback set. Every insight must point back to real customer evidence.'}</p></div></div><div className="ai-meta"><CircleCheck size={15}/> Evidence linked to every recommendation</div><button className="secondary" onClick={runAnalysis} disabled={analysis}>{analysis?'Analyzing...':'Analyze with AI'} <ChevronRight size={15}/></button></div></section><OpportunityTable onClick={()=>setPage('Prioritization')} /></> }
...
function FeedbackPage({feedback}){return <div className="panel feedback-panel"><div className="panel-heading"><div><h2>Customer feedback</h2><p>Raw evidence stays visible so AI insights can be challenged.</p></div><span className="feedback-count">{feedback.length} items</span></div><div className="feedback-list">{feedback.map(item=><div className="feedback-item" key={item.id}><div className="feedback-icon"><FileText size={16}/></div><div className="feedback-content"><p>“{item.text}”</p><div className="feedback-meta"><strong>{item.customer}</strong><span>{item.source}</span><span>{item.date}</span></div></div></div>)}</div></div>}
